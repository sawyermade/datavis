class bar
{
  ...
  bar(Table data, int idx0, int idx1)
  {
    this.data = data;
    this.idx0 = idx0;
    this.idx1 = idx1;
  ...
  void  draw_bar(mousepoint p)
  {
    ...
    for(i = 0; i < data.getRowCount();i++)
    {
      ...
      if(p != null)
      {
          ...
          if(p.box == 4)
          {
            if(p.x + w <= x + 4 && p.x +  w >= x - 4 && p.y - h <= y + 4 && p.y -h >= y - 4)
          {
               stroke(53, 163, 101);
               strokeWeight(3);
          }
        ...
        rect(x,y,(w - 2 * spacer)/data.getRowCount(),v0 + h - spacer -y);
