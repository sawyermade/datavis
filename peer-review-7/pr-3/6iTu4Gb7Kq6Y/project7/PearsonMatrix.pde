...
class PearsonMatrix extends Frame {
  
    ArrayList<Pearson> plots = new ArrayList<Pearson>( );
    int colCount;
    Table data;
    float border = 10;
    
   PearsonMatrix( Table data, ArrayList<Integer> useColumns ){
     this.data = data;
     colCount = useColumns.size();
     for( int j = 0; j < colCount-1; j++ ){
       for( int i = j+1; i < colCount; i++ ){
           Pearson sp = new Pearson( table, useColumns.get(j), useColumns.get(i) );
           plots.add(sp);
       }
     }
