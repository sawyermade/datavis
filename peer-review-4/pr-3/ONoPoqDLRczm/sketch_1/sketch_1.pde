...
void keyPressed()
{
  textSize(15);
  fill(255, 0, 0);
  text("Click canvas & Press below key(hover on top of bars):",2,12);
  fill(0, 102, 153);
   text("Press A for "+ table.getColumnTitle(0) +" Vs " +table.getColumnTitle(1),10+2,27);
  text(" Press B for "+ table.getColumnTitle(1) +" Vs " +table.getColumnTitle(2),10+2,42);
...
input=1;
    ...
    myFrame = new bar( table, useColumns, 4);
  }
}
...
void draw(){
  ...
  keyPressed();
  if (keyPressed) {
     myFrame = new bar( table, useColumns, input);
  }
}
